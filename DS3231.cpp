...
void DS3231::enableOscillator(bool TF, bool battery, byte frequency) {
    ...
    if (frequency > 3) frequency = 3; // Make sure the highest frequency is no more than 3
...
bool DS3231::CheckEOSC() {
    // Return false ~ Oscillator will stop when using Battery Power.
    //
    byte temp_buffer = readControlByte(0);
    bool result = true;
    if (temp_buffer & 0b10000000) {
        // EOSC Flag is set, so return false.
        result = false;
    }
    return result;
}

bool DS3231::CheckINTCN() {
    // Return false ~ Oscillator will stop when using Battery Power.
    //
    byte temp_buffer = readControlByte(0);
    bool result = true;
    if (temp_buffer & 0b00000100) {
        // INTCN Flag is set, so return false.
        result = false;
    }
    return result;
}

void DS3231::enableINTCN(bool INT) {
    // turn INTCN on or off
    byte temp_buffer = readControlByte(0);
    if (INT) {
        // turn on
